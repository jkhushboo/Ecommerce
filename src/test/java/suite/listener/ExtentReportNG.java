...
public class ExtentReportNG implements IReporter {
	...
	private void init(String outputDirectory) {
		ExtentSparkReporter htmlReporter = new ExtentSparkReporter(outputDirectory + File.separator + "Extent.html");
		...
		extent.setSystemInfo("Test Environment", "PAS Environment");
		extent.setSystemInfo("Machine Name", "AMAZUQAW10");
		extent.setSystemInfo("User Name:", "Khushboo Jain");
				...
				if (result.getThrowable() != null) {
					...
					try {
						...
						if (readProperties("env").equalsIgnoreCase("local")) {
							finalPath = Paths.get("").toAbsolutePath().toString() + readProperties("localReportPath")
									+ result.getName() + ".png";
						} else {
						...
						test.fail("Screenshot is as below:",
